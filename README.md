...
- **Batch, not one-at-a-time.** Export your entire **Bookmarks**, **Likes**, a **Profile**, your home **Timeline**, or a hand-picked **Selection** in a single background job, with live progress, stop, and dedup — plus the optional **Auto** and **Super** engines that pull items straight from your X session for a roughly **10× speed-up**.
...
- **Six export formats** — Save a single post as **Markdown, PDF, HTML, JSON, TXT, or CSV**, or hand it to **Obsidian**. Batch jobs support every format except PDF. CSV pairs your metadata columns with a `text` column for the post body.
