...
- **Auto and Super no longer repeat a link the card already shows**: a post ending in a link rendered the link *and* the card for it, one above the other with the same destination — X hides that link on the page for exactly that reason, and Manual followed suit, so the two engines disagreed. The link is now dropped when a card represents it; a link with no card still renders. (#126)
- **Auto and Super no longer prefix a threaded post with the author's own @mention**: continuing your own thread makes X put the replied-to account at the front of the post text, which the page renders as reply context rather than as words you wrote. Manual never showed it and Auto did, so the same thread read `@Google Learn more` on one engine and `Learn more` on the other. Auto and Super now follow the range X marks as the displayed text. (#126)
- **Card thumbnails now use the same image size on every engine**: Auto and Super saved a card's preview at the layout-specific size X serves it at while Manual saved the large variant, so the same post yielded two different image files. Both now request the large one. (#126)
- **Auto and Super now count reposts the way X displays them**: the `reposts:` value changed with the engine by far more than two runs minutes apart could explain — one post read 231 on Manual and 165 on Auto. X's own UI shows reposts and quote posts as a single number, which is what Manual reads off the page, while Auto and Super read only the repost half of the pair X sends separately. They now sum the two, so both engines report the number you see on the post (165 reposts + 66 quotes = the 231 Manual already showed). One difference remains and can't be closed from the page: X displays large counts rounded, so Manual still reports `1.6K` as `1600` where Auto has the exact `1617`. (#125)
- **Auto and Super now write the `@` in the `handle` frontmatter field**: the same post exported with different engines wrote a different handle — `handle: "@NotionHQ"` on Manual, `handle: "NotionHQ"` on Auto and Super — so a vault built from two runs held two spellings of the same author, and the Obsidian `author:` wikilink pointed at two different notes. Every path that turns a post into an export now goes through one conversion that writes the `@`, matching what Manual and single export have always produced; the filename and the tag template keep stripping it as before. Exports made with Auto or Super before this change keep the bare form. (#124)
...
- **Auto and Super now keep a post embedded in an X Article**: a post quoted inside an X Article body reached the export on Manual but vanished on Auto and Super — the text ran straight from the paragraph before the embed to the one after, and the embedded post's video thumbnail never made it into the media folder either. X names an embedded post by id and sends none of its content with the article, so Auto and Super now fetch each one and place the full post exactly where Manual does. If a post can't be fetched the export keeps a link to it rather than leaving a hole; when X's rate limit is what got in the way, the article is filed under `_incomplete_rerun_to_complete/` and left unmarked, so the next run replaces the link with the real post. (#123)
